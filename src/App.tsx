          ...
          {navItems.map((item) => (
            <div key={item.label} className="relative group">
              <button 
                ...
                className={`flex items-center gap-1 hover:text-brand-red transition-colors ${activeDropdown === item.label ? 'text-brand-red' : ''}`}
              >
          ...
          {isAuthenticated ? (
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              <Link to="/login" className="text-white/70 hover:text-brand-red transition-colors flex items-center gap-2">
                <User size={18} />
                <span className="hidden lg:inline text-[9px] font-bold uppercase tracking-widest">Login</span>
              </Link>
              <Link to="/signup" className="hidden sm:flex btn-primary py-2 px-5 text-[9px] font-black uppercase tracking-widest italic">
                Join
              </Link>
            </div>
          )}
                  ...
                  <div className="grid grid-cols-1 gap-4 pl-4">
                    {item.sub.map((sub) => (
                      <Link 
                        ...
                        className="text-lg font-display font-bold uppercase italic text-white/70 hover:text-white flex items-center justify-between group"
                      >
                        {sub.label}
